...
\placeholder{review}
	...
	\paragraph{Effect.}
	Enabling FLTrust increases model accuracy and mitigates the effect of the label-flipping attack, as shown in Figure~\ref{fig:confusion_matrices}. The flipped labels, digits 3 and 7, are consistently flipped in the predictions from the basic model and correctly predicted when enabling FLTrust. The effect increases with larger Byzantine fractions and larger attack scales. The depicted results were achieved using a Byzantine fraction of $\eta = 40\%$ and an attack scale of 2.0. 
	
	\paragraph{Cost.}
	At a setting of 50 clients and 50 training rounds, applying FLTrust resulted in a wall-clock training time of 29.4\%. Smaller numbers of clients yielded a smaller percentual increase, down to no noticeable impact at 10 clients and 5 training rounds.
	...
	\section{APPENDIX}
		...
		\caption{Final confusion matrices for the MNIST dataset for standard FedAvg (top) and with FLTrust enabled (bottom).}
